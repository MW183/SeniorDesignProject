...
export default function TaskEditor({
  ...
  const loadTaskNotes = async (taskId: string) => {
    // TaskNotes are already loaded from the API, just toggle expansion
      ...
      if (res.ok) {
        ...
        const updatedTasks = tasks.map(t => {
          if (t.id === taskId) {
            return {
              ...t,
              taskNotes: [res.body, ...t.taskNotes] // Add new note to the beginning (newest first)
            };
          }
          return t;
        });
            ...
            {isEditing ? (
              ...
              <div onClick={() => startEditingTask(task)}>
                ...
                <div className="mt-3 pt-3 border-t border-border" onClick={(e) => e.stopPropagation()}>
                  ...
                  >
                    {`${task.taskNotes?.length || 0} note${task.taskNotes?.length !== 1 ? 's' : ''}`}
                  </button>
